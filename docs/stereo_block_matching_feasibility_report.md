...
```text
       ...
       Nios V instruction/data managers
                    |
                    v
        AXI/interconnect adaptation as
        generated by Platform Designer
                    |
                    +-------------------------------------------+
                    |                                           |
                    v                                           v
          Shared SDRAM controller                   Accelerator control slave
                    ^                                 status/configuration
                    |
                    +-------------------------------------------+
                                      Accelerator memory master

Shared SDRAM regions:
  - left input buffers
  - right input buffers
  - best-cost/intermediate buffers if required
  - output disparity buffers
  - frame descriptors and status
```
...
### 5.3 Illustrative control-register map
...
```text
Avalon-MM burst reader
        |
        v
Input FIFOs
        |
        v
Left/right line and horizontal-history buffers
        |
        v
Parallel absolute-difference lanes
        |
        v
Window SAD accumulation
        |
        v
Winner-take-all minimum selection
        |
        v
Output FIFO
        |
        v
Avalon-MM burst writer
```

### 6.2 Why line buffers are necessary
...
### 6.4 Parameterized disparity lanes
...
platform initialization
    |
    v
SDRAM memory test / optional startup check
    |
    v
load or generate stereo dataset into shared SDRAM
    |
    +-----------------------------+
    |                             |
    v                             v
CPU-only SAD benchmark       accelerator benchmark
    |                             |
    v                             v
CPU disparity map            FPGA disparity map
    |                             |
    +-------------+---------------+
                  v
